...
int main()
{
	...
	while (newWFF == true)
	{
		...
		int parentheses = 0;	//if parentheses isn't 0 at the end of the WFF, then ya done messed up.  
		...
		for (char c : input)
		{
			...
			else if (c == '(' && isConnector == true || c == '(' && isAlpha == false && isConnector == false && lastWasNot == false && lastWasDash == false) // the OR part allows the user to put parentheses before the first letter
			{																																				//ex: (a v k) ^ m
				parentheses++;
			}
			else if (c == ')' && isAlpha == true)
		...
		if (fail == false && isAlpha == true && parentheses == 0)	//the mess above only checks to see if the WFF is valid up to the point c is at.  isAlpha here will
